...
def auto_register(password: str):
	while True:
		...
		try:
			...
			client.register(email=email, password=password, nickname=nickname, verification_code=verification_code)
			print(f"{email} registered and activated")
			emails = open("emails.txt", "a+")
			emails.write(f"Email: {email}, Password: {password}, deviceId: {device_Id}\n")
			emails.close()
